$(document).ready(function() {
		...
		buttons: [{
				  ...
				  },
				  {
					 ...
					 action: function ( e, dt, node, config ) {
						 ...
						 txtCoQuanTuyenDungAdd.val("cty sg");
						 txtNgayTuyenDungAdd.val("");
						 txtChucVu_pkAdd.val("");
						 txtCongViecDuocGiaoAdd.val("nhân viên");
						 txtChucDanhAdd.val("Nhân Viên");
	...
	changeNgachCongChuc = function(){
		...
		$.ajax({  
		  ...
		  success: function (res) {
			  ...
			  $.each(res.bacLuong,function(i,o){
				  toAppend += '<input value='+o.pk+'>'+o.maBacLuong+'</imput>';
			 });
			
			 $('#bacLuong_pk').val(res.pkbacluong);
		...
		changeBacLuongsy = function() {
		var bacLuongPk = $('#bacLuong_pk').val();
		$.ajax({
			url: canBoService + "/getHeSoLuongById/" + bacLuongPk,
			type: 'GET',
			success: function(res) {
				if (res && res.heSoLuong) {
				
					$('#heSoLuong').val(res.heSoLuong);
				} else {
					
					$('#heSoLuong').val('Không tìm thấy hệ số lương');
				}
			},
			error: function() {	$('#heSoLuongAdd').val('Không tìm thấy hệ số lương');
				$('#heSoLuong').val('Không tìm thấy hệ số lương');
			}
		});
	};
	changeNgachCongChucAdd = function(){
		 ...
		 $.ajax({  
		   ...
		   success: function (res) {
			  ...
			  $('#bacLuong_pkAdd').val(res.pkbacluong);
			  $('#bacLuong_pk').val(res.pkbacluong);
		...
		$.ajax({
			...
			success: function(res) {
				if (res && res.heSoLuong) {
					...
					$('#heSoLuongAdd').val('Không tìm thấy hệ số lương');
					$('#heSoLuong').val('Không tìm thấy hệ số lương');
				}
			},
			error: function() {
