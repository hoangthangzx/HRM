...
public class QuyetDinhKhenThuongServiceImpl implements
	...
	@Override
    public List<QuyetDinhKhenThuongDTO> getAllByCanBoPk(Integer canBoPk) {
        List<Quyetdinhkhenthuong> listFromDb = repo.findAllByCanBoPk(canBoPk);
        List<QuyetDinhKhenThuongDTO> ketqua = new ArrayList<>();
        
        for (Quyetdinhkhenthuong d : listFromDb) {
            DonViChucNangDTO donViChucNangDto = mapper.map(d.getDonvichucnang(), DonViChucNangDTO.class);
            CanBoDTO canBoDto = mapper.map(d.getCanbo(), CanBoDTO.class);

            QuyetDinhKhenThuongDTO QuyetDinhKhenThuongDTO = mapper.map(d, QuyetDinhKhenThuongDTO.class);
            QuyetDinhKhenThuongDTO.setDonViChucNang(donViChucNangDto);
            QuyetDinhKhenThuongDTO.setCanBo(canBoDto);
